...
public class Category_detail_Data_Factory {
    ...
    public Category_detail_Data_Factory(String[][] array, String[][] codeArray){
        for(int i = 0; i < array.length; i++){
            ArrayList<Category_Children> childrenitem = new ArrayList<>();
            for(int j = 1; j < array[i].length; j++){
                Category_Children categoryChildren = new Category_Children((String)array[i][j], (String)codeArray[i][j]);
